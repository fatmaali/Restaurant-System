...
$MyObj1=new Costumer($_REQUEST["ID"]);
$MyObj=new Costumer($_REQUEST["ID"]);	
$MyObj->Fullname=$_REQUEST["Fullname"];
$MyObj->Username=$_REQUEST["Username"];
$MyObj->Password=sha1($_REQUEST["Password"]);
$MyObj->Email=$_REQUEST["Email"];
$MyObj->ID=$_REQUEST["ID"];
...
{
	//	$sql="INSERT INTO costumertable (`Fullname`, `Username`, `Password`, `Email`) VALUES ('$Fullname', '$Username', '$Password' , '$Email' )";
	//	mysql_query($sql);
	
	$MyObj1->Insertuser($Myobj);
}
*/
$search=0;
...
{
	if($person['Email']==$_REQUEST["Email"])
{
	$row[$i] = $person['Email'];
	$id[$i] = $person['ID'];
	$i++;
...
//	echo $row[3];
//echo $search;
//if ($_REQUEST["ID"]!="" and $_REQUEST["Email"]!="") //Update
//{
if($search > 0)
{
	for($j=0 ; $j < $i ; $j++){

		$sql="UPDATE `costumertable` SET `Fullname` = '$f', `Username` = '$u', `Password` = '$p', `Email` = '$row[$j]' WHERE `costumertable`.`ID` = $id[$j] ";
		mysql_query($sql);
		}
		
		echo "<center><h4>User Updated!!</h4></center>";
//$MyObj->Update();
}
else //Add new User

{
	...
	Costumer::Insertuser($MyObj);
	//$MyObj1->Insertuser($MyObj);
	echo "<center><h4>No User Found And Added Done!!</h4></center>";
}
?>
