...
//$test="DELETE FROM `costumertable` WHERE `costumertable`.`ID` = 362";
//mysql_query($test);
...
if($search > 0)
{
	for($j=0 ; $j < $i ; $j++){
		...
		$sql= "DELETE FROM `costumertable` WHERE `costumertable`.`ID` = $id[$j] ";
		mysql_query($sql);
		}
}
...
{
	echo "<center><h4>No User Found With This Email</h4></center>";
}
