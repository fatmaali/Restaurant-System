...
static function selectAllItems()  //select all provider's order by id 
	{
	 $DBObj = new Connection;
	 
	 $sql="SELECT * FROM item";
	 $res= $DBObj->Conn->query($sql);
	 $i=0;
	 $Result;
	foreach($res as $rows)
	{
...
//////////////////////////////////////////////////////////////////// User ///////////////////////////////////////////////////////////////////
		...
		{
		  $sql = "update user set fname='".$U->getUserFname()."',lname='". $U->getUserLname()."',mobile='".$U->getUserMobile()."',gender='".$U->getUserGender()."',gov='".$U->getGov()."' WHERE userID=".$U->getUserID();
		...
		{
		  $sql = "update user set password='".$User->getUserPassword()."' WHERE userID=".$User->getUserID();
...
///////////////////////////////////////////////////////////////////Provider//////////////////////////////////////////////////////////////////
	...
	{
		$sql="SELECT * FROM user WHERE userType='provider'";
		$res= $this->Conn->query($sql);
		$i=0;
		$Result;
		foreach($res as $rows)
		{
			$ProviderObj=new User($rows['userID']);
			$Result[$i]=$ProviderObj;
			$i++;
		}
		return $Result;	 			
...
///////////////////////////////////////////////////////////////////Menu//////////////////////////////////////////////////////////////////////
	...
	public function selectProvIDByMenu($MID)  //select all provider's order by id 
	{
	 $DBConn = new Connection;
	 
	 $sql="SELECT ProviderID FROM menu WHERE menuID=".$MID;
	 $res= $this->Conn->query($sql);
	...
	public function selectMenuIdByPID($id) 
	{
		$s="SELECT * FROM menu WHERE ProviderID =".$id;
		$res = $this->Conn->prepare($s); 
		$res->bindParam(1, $id);
		$res->execute();
		$result = $res->fetchAll();
...
///////////////////////////////////////////////////////////////////Review////////////////////////////////////////////////////////////////////	
	...
	public function selectReview($userID,$itemID)
	{
		$s="SELECT ID FROM review WHERE clientID = ? and itemID=?";
		$res = $this->Conn->prepare($s); 
		$res->bindParam(1, $userID);
		$res->bindParam(2, $itemID);
		$res->execute();
		$result = $res->fetchAll();
		$r=null;	
		foreach($result as $rows)
		{
		  $r=new review($rows['ID']);
		}
		return $r;
	}
